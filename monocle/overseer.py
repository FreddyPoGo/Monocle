...
class Overseer:
    ...
    def start(self, status_bar):
        ...
        self.blind_list = self.manager.blind_list()
        Worker.blind_list = self.manager.blind_list()
        if conf.MAP_WORKERS:
        ...
        for username, account in ACCOUNTS.items():
            ...
            if account.get('shadow_banned'):
                self.blind_list.append(account['username'])
    ...
    def update_stats(self, refresh=conf.STAT_REFRESH, med=median, count=conf.GRID[0] * conf.GRID[1]):
        ...
        self.stats = (
            ...
            'Extra accounts: {}, Blind accounts: {}, CAPTCHAs needed: {}\n'
        ).format(
            ...
            self.extra_queue.qsize(), len(self.blind_list), self.captcha_queue.qsize()
        )
