...
from .shadowbans import is_rareless_scan
...
class Worker:
    ...
    def __init__(self, worker_no):
        ...
        self.shadow_banned = self.account.get('shadow_banned', False)
    ...
    async def visit_point(self, point, spawn_id, bootstrap,
        ...
        if is_rareless_scan(map_objects):
            await self.mark_blind()
        else:
            await self.unmark_blind()
            ...
            for fort in map_cell.forts:
                ...
                if fort.type == 1:  # pokestops
                    ...
                    if fort.HasField('raid_info'):
                        ...
                        if notify_conf and self.notifier.raid_eligible(normalized_raid):
    ...
    def update_accounts_dict(self):
        ...
        self.account['shadow_banned'] = self.shadow_banned
        if self.player_level:
    ...
    async def mark_blind(self):
        self.error_code = 'BLINDED'
        self.log.warning('{} is shadow banned.', self.username)
        if self.username not in self.blind_list:
            self.shadow_banned = True
            self.blind_list.append(self.username)
            self.update_accounts_dict()

    async def unmark_blind(self):
        if self.username in self.blind_list:
            self.shadow_banned = False
            self.blind_list.remove(self.username)
            self.update_accounts_dict()
