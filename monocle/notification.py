...
class Notifier:
    ...
    async def webhook(self, pokemon):
        ...
        try:
            ...
            data['message']['cp_multiplier'] = pokemon['cp_multiplier']
        except KeyError:
            pass
    ...
    async def raid_webhook(self, raidinfo, lat, lon, team):
        ...
        if conf.RAID_WEBHOOK:
            ...
            session = SessionManager.get()
            await self.hook_post(conf.RAID_WEBHOOK, session, payload)
        
        data = {
            'type': "raid",
            'message': {
                "gym_id": b64encode(raidinfo['external_id'].encode('utf-8')),
                "raid_seed": raidinfo['raid_seed'],
                "pokemon_id": raidinfo.get('pokemon_id', 0),
                "cp": raidinfo.get('cp', 0),
                "move_1": raidinfo.get('move_1', 0),
                "move_2": raidinfo.get('move_2', 0),
                "spawn": raidinfo['raid_spawn'],
                "start": raidinfo['raid_start'],
                "end": raidinfo['raid_end'],
                "level": raidinfo['raid_level'],
                "latitude": lat,
                "longitude": lon
            }
        }
        ...
        self.raid_cache.add(cache_key, 7300)
